...
class Employee:
    def __init__(self, emp):
        ...
        self.button5.configure(text="UNDO")
        self.button5.configure(command=self.undo_queue) """
    ...
    """ def undo_queue(self):
        if self._prevtruth == True:
            self._que.delete_first()
        elif self._prevtruth == False:
            self._que.delete_last()
        x = self._que.__str__()
        f = open("patientqueue.csv","w",newline="")
        a = csv.writer(f)
        a.writerows(x)
