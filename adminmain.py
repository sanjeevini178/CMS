...
import json
'''
Title: Ex 8 Mini Project
Author: Sanjeevini R
Date: 13/01/2023
Concept Used: Iterator and Singleton Design Patterns
'''
def valid_phone(phn):
...
class CapitalIterable(list):
    def __init__(self, list):
        self.list = list

    def __iter__(self):
        return CapitalIterator(self.list)

#ITERATOR PATTERN
class CapitalIterator(list):
    def __init__(self, list):
        self.list=list
        self.index = 0
...
class Employee:
    ...
    def __new__(cls):
        if (cls.__instance is None):
            cls.__instance = super(Employee,cls).__new__(cls)
    ...
    def DisplayData(self):
        global acc
        f=open("userrecord.json","r")           #USE OF JSON SERIALIZATION
        a=json.load(f)
        b=a.items()
        acc=[[i[0]]+i[1] for i in b]   

        iterable = CapitalIterable(acc)                     #USE OF ITERATOR
        iterator = iter(iterable)
        while True:
            try:
                self.tree.insert("", "end", values=(next(iterator)))
    ...
    def delete_emp(self):
        ...
        if len(self.sel)!=0:
            ...
            if sure == True:
                ...
                for i in acc:
                    dict[i[0]]=i[1:]
                f2=open("userrecord.json","w",newline="")           #USE OF JSON SERIALIZATION
                json.dump(dict,f2)
                f2.close()
            ...
            if valid_phone(enum):
                if eun:
                    if eacctype:
                        if "@" in eadd:
                            if valid_pwd(epass):
                                f1=open("userrecord.json","r",newline="")               #USE OF JSON SERIALIZATION
                                data=json.load(f1)
                                b=data.items()
                                acc=[[i[0]]+i[1] for i in b]
...
def page():
    ...
    print("Two objects 'a' and 'b' for Employee have been created")
    print(f"Type of object 'a': {type(a)}")
    print(f"Type of object 'b': {type(b)}")
    print(f"Memory address of object 'a': {id(a)}")
    print(f"Memory address of object 'b': {id(b)}")
    print("Due to the usage of Singleton pattern, the same memory address has been assigned for both the objects.")
    a.mainf(emp)
    emp.mainloop()
